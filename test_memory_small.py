...
def test_mem(grad_ckpt=False, dummy_grad=False):
    model.train()
    model.encoder.gradient_checkpointing = grad_ckpt
    model.decoder.gradient_checkpointing = grad_ckpt
    
    # Optional hook to force requires_grad
    if dummy_grad:
        def make_requires_grad(module, input, output):
            output.requires_grad_(True)
        h1 = model.encoder_embedding.register_forward_hook(make_requires_grad)
        h2 = model.decoder_embedding.register_forward_hook(make_requires_grad)
    ...
    try:
        ...
        print(f"ckpt={grad_ckpt}, force_grad={dummy_grad} | Fwd: {fwd_mem-start_mem:.2f} GB | Bwd: {bwd_mem-start_mem:.2f} GB")
    except Exception as e:
        print(f"ckpt={grad_ckpt}, force_grad={dummy_grad} | Error: {e}")
