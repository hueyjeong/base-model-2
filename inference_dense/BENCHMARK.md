...
|---|---|---|---|---|
| **Mamba-2 ds=64** | **0.107** | **93.1%** | **71.0%** | **압도적 1위** |
| Mamba-1 | 0.170 | 89.1% | 51.5% | 2위 |
| RWKV | 0.212 | 82.0% | 38.0% | 실용 가능 |
| TCN | 0.222 | 87.0% | 32.4% | 실용 가능 |
| RetNet | 0.297 | 92.1% | 0.8% | 편집 불능 (KEEP만 예측) |
| xLSTM | 0.303 | 97.5% | 0.7% | 편집 불능 (KEEP만 예측) |
| FNet | 5.315 | 4.0% | 45.6% | loss 발산 (학습 실패) |

**Mamba-2 ds=64가 Mamba-1 대비: loss 37% 낮음, recall +19.5pp (51.5→71.0%), precision +4pp**
**써먹을 놈**: Mamba-2, Mamba-1, RWKV, TCN — **못 써먹을 놈**: RetNet, xLSTM, FNet
...
**최종 확정: Mamba-2 ds=64** — CPU 22% 빠름, 10k 품질 Mamba-1 압도 (loss 37%↓, recall +19.5pp)
CPU 멀티코어 최적인 mLSTM/RetNet/xLSTM는 품질이 치명적 (recall <1%).

### Mamba-2 품질 비교 (2000-step, seq=512, batch=8, 싱글GPU)

| Arch | Loss | P | R | CPU 8T (ms) |
|---|---|---|---|---|
| Mamba-1 | 0.185 | 94.6% | 32.4% | 1,481 |
| **Mamba-2 ds=64** | **0.159** | **95.1%** | **36.1%** | **1,159** |
| Mamba-2 ds=128 | 0.159 | 94.3% | 30.6% | ~1,480 |

Mamba-2 ds=64이 loss 14% 낮고, recall 11% 높으며, CPU 22% 빠름.
ds=128은 loss 동일하나 recall 열세 + CPU 속도 이점 없음 → **ds=64 확정**.
