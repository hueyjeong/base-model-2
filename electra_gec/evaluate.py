...
def evaluate(args):
    ...
    # ── 모델 로드 ──
    model = KoELECTRAGECToR(args.model_name, single_head=args.single_head, tied_content=not args.no_tied_content).to(device)
    if args.checkpoint:
...
def main():
    ...
    p.add_argument("--no_tied_content", action="store_true", help="Two-head untied content head")
