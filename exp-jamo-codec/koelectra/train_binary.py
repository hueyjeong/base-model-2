"""Binary ELECTRA 사전학습 스크립트 (변종 A: pure binary, k=0).

Codec 완전 제거. BBPE token id → 18-bit binary 직접 입력.

사용 예시 (4 GPU DDP):
    torchrun --nproc_per_node=4 -m koelectra.train_binary \
...
from koelectra.binary_electra import BinaryElectra  # noqa: E402
from koelectra.data.bbpe_dataset import BBPEDataset, load_bbpe_tokenizer  # noqa: E402
from koelectra.data.masking import make_patch_mask  # noqa: E402
...
def apply_mask_bbpe(bbpe_ids, masked_patch_mask, mask_id):
    return torch.where(masked_patch_mask, mask_id, bbpe_ids)
...
def run_validation(model, val_dataset, args, device, amp_dtype, rank, world_size,
    ...
    sums = {k: torch.zeros(1, device=device) for k in
            ("gen", "disc", "total", "disc_acc", "gen_acc", "bit_acc", "util")}
    ...
    for i, batch in enumerate(loader):
        ...
        masked_bbpe_ids = apply_mask_bbpe(bbpe_ids, masked_patch_mask, mask_id)

        with torch.amp.autocast("cuda", dtype=amp_dtype, enabled=(device.type == "cuda")):
            out = model(bbpe_ids, token_pad_mask, masked_bbpe_ids, masked_patch_mask)
        sums["gen"] += out["gen_loss"].detach().float()
        sums["disc"] += out["disc_loss"].detach().float()
        sums["total"] += out["total_loss"].detach().float()
        sums["disc_acc"] += out["disc_acc"].detach().float()
        sums["gen_acc"] += out["gen_acc"].detach().float()
        sums["bit_acc"] += out["bit_acc"].detach().float()
        sums["util"] += out["patch_util"].detach().float()
        count += 1.0
    ...
    model.train()
    return {f"val/{k}": (v / count).item() for k, v in sums.items()}
...
def main():
    ...
    # Model
    ap.add_argument("--vocab_size", type=int, default=153600)
    ap.add_argument("--bbpe_bits", type=int, default=18)
    ap.add_argument("--embedding_size", type=int, default=128)
    ap.add_argument("--hidden_size", type=int, default=256)
    ap.add_argument("--n_heads", type=int, default=4)
    ap.add_argument("--d_ff", type=int, default=1024)
    ap.add_argument("--gen_layers", type=int, default=14)
    ap.add_argument("--disc_layers", type=int, default=14)
    ...
    # 체크포인트 & 로깅
    ap.add_argument("--out_dir", type=str, default="exp-jamo-codec/koelectra/checkpoints_binary")
    ...
    if is_rank0(rank):
        ...
        print(f"[Setup] vocab={args.vocab_size}, bits={args.bbpe_bits}, "
    ...
    t0 = time.time()
    accs = {k: 0.0 for k in
            ("total", "gen", "disc", "disc_acc", "gen_acc", "bit_acc", "rep", "mask", "util")}
    acc_count = 0
    ...
    while global_step < args.max_steps:
        for micro in range(args.grad_accum_steps):
            ...
            accs["total"] += out["total_loss"].detach().float().item()
            accs["gen"] += out["gen_loss"].detach().float().item()
            accs["disc"] += out["disc_loss"].detach().float().item()
            accs["disc_acc"] += out["disc_acc"].detach().float().item()
            accs["gen_acc"] += out["gen_acc"].detach().float().item()
            accs["bit_acc"] += out["bit_acc"].detach().float().item()
            accs["rep"] += out["replaced_rate"].detach().float().item()
            accs["mask"] += out["masked_tokens"].detach().float().item()
            accs["util"] += out["patch_util"].detach().float().item()
            acc_count += 1
        ...
        if is_rank0(rank) and global_step % args.log_every == 0:
            ...
            msg = (f"step {global_step:>7d} | "
                   f"total {accs['total']/n:.3f} | "
                   f"gen {accs['gen']/n:.3f} | "
                   f"disc {accs['disc']/n:.3f} | "
                   f"disc_acc {accs['disc_acc']/n:.3f} | "
                   f"gen_acc {accs['gen_acc']/n:.3f} | "
                   f"bit_acc {accs['bit_acc']/n:.3f} | "
                   f"rep {accs['rep']/n:.3f} | "
                   f"mtok {accs['mask']/n:.0f} | "
                   f"util {accs['util']/n:.3f} | "
                   f"lr {lr_main:.2e} | "
        ...
        if val_ds is not None and global_step % args.val_every == 0:
            ...
            val_metrics = run_validation(
                ...
                mask_id=mask_id, n_batches=args.val_batches,
            )
            if is_rank0(rank):
        ...
        if global_step % args.save_every == 0:
            ckpt_path = os.path.join(args.out_dir, f"binary_step_{global_step}.pt")
    ...
    final_path = os.path.join(args.out_dir, f"binary_step_{global_step}_final.pt")
