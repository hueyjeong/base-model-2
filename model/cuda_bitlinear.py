...
def _quantize_weights_int8(w: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """가중치를 INT8 absmax 양자화 (활성화와 동일 방식)"""
    eta = w.abs().max().clamp(min=1e-5)
    w_q = (w * 127.0 / eta).round().clamp(-128, 127)
    scale = eta / 127.0
    return w_q, scale
...
class _Int8LinearCudaFn(torch.autograd.Function):
    ...
    @staticmethod
    @torch.amp.custom_fwd(device_type="cuda")
    def forward(ctx, x_norm, weight, bias, out_dtype):
        x_q, x_scale = _quantize_activations(x_norm.float())
        w_q, w_scale = _quantize_weights_int8(weight.float())
...
class Int8LinearCuda(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return _Int8LinearCudaFn.apply(x_norm, weight, bias, out_dtype)

    def extra_repr(self) -> str:
        return (
            f"in={self.in_features}, out={self.out_features}, "
            f"bias={self.bias is not None}, quant=int8+cuda-int8"
        )
